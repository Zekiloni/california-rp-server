...
mp.events.add({
    'playerEnterVehicle': (player, vehicle, seat) => {
        ...
        if (vehicle.info) {
            if (vehicle.info.job && vehicle.info.job != character.job)
                return player.removeFromVehicle();
            if (vehicle.info.faction && vehicle.info.faction != character.faction)
                return player.removeFromVehicle();
        }
        if (seat == 0) {
            player.call('client:player.vehicle', [true, vehicle.engine]);
        }
    },
    'playerStartExitVehicle': (Player) => {
        if (Player.vehicle.engine)
            Player.vehicle.engine = true;
        if (Player.data.Seatbelt)
    ...
    'server:vehicle:indicators': (player, indicator) => {
        ...
        if (vehicle && player.seat == 0) {
            switch (indicator) {
                case 0:
                    vehicle.setVariable('IndicatorRight', !vehicle.getVariable('IndicatorRight'));
                    break;
                case 1:
                    vehicle.setVariable('IndicatorLeft', !vehicle.getVariable('IndicatorLeft'));
                    break;
            }
        }
    },
    'server:vehicle:interaction': (Player, Vehicle, Action) => {
        ...
        switch (Action) {
            case 'bonnet': {
                if (Vehicle.locked)
                    return Player.Notification(frp.Globals.messages.VEHICLE_IS_LOCKED, frp.Globals.Notification.Error, 6);
