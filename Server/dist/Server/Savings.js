    ...
    constructor() {
        mp.events.add({
            'playerQuit': async (player, exit) => {
                switch (exit) {
                    case 'disconnect': {
                        frp.Main.Terminal(2, player.name + ' left the server.');
                        break;
                    }
                    case 'timeout': {
                        frp.Main.Terminal(2, player.name + ' lost the connection to the server.');
                        break;
                    }
                    case 'kicked': {
                        frp.Main.Terminal(2, player.name + ' has been kicked / banned from the server.');
                        break;
                    }
                }
