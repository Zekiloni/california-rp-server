    ...
    commands: [
        ...
        },
        {
            ...
            call: async (player, args) => {
                ...
                if (!action)
                    return player.Notification(frp.Globals.messages.COMMAND_USAGE + '/channel join, leave, create, password', frp.Globals.Notification.Info, 4);
                const Character = await player.Character();
                switch (action) {
                    case 'join':
                        frp.Channels.Join(player, data, value);
                        break;
                    case 'leave':
                        frp.Channels.Leave(player);
                        break;
                    case 'create':
                        frp.Channels.New(player, data, value);
                        break;
                    case 'password':
                        frp.Channels.Edit(player, data);
                        break;
                    case 'delete':
