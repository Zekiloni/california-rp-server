...
mp.events.add({
    'playerEnterVehicle': (Player, Vehicle, Seat) => {
        ...
        if (Vehicle.Job && Vehicle.Job != Character.Job)
            return Player.removeFromVehicle();
        if (Vehicle.Faction && Vehicle.Faction != Character.Faction)
            return Player.removeFromVehicle();
    },
    'playerStartExitVehicle': (Player) => {
        if (Player.vehicle.engine)
            Player.vehicle.engine = true;
        if (Player.getVariable('Seatbelt'))
    ...
    'server:vehicle:indicators': (Player, Indicator) => {
        if (Player.vehicle && Player.seat == 0) {
            switch (Indicator) {
                case 0:
                    Player.vehicle.setVariable('IndicatorRight', !Player.vehicle.getVariable('IndicatorRight'));
                    break;
                case 1:
                    Player.vehicle.setVariable('IndicatorLeft', !Player.vehicle.getVariable('IndicatorLeft'));
                    break;
            }
        }
    },
    'server:vehicle:interaction': (Player, Vehicle, Action) => {
        ...
        switch (Action) {
            case 'bonnet': {
                if (Vehicle.locked)
                    return Player.Notification(Messages_1.Messages.VEHICLE_IS_LOCKED, Globals_1.Globals.Notification.Error, 6);
    ...
    'server:vehicle:update': async (Player, Vehicle, Mileage, Fuel) => {
        if (Vehicle.DATABASE) {
            const iVehicle = await Vehicles_1.Vehicles.findOne({ where: { id: Vehicle.DATABASE } });
            if (iVehicle) {
