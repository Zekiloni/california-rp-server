...
frp.Taxi = (_a = class Taxi {
        ...
        static async Start(Player, haveVehicle) {
            ...
            if (haveVehicle == null) {
                ...
                if (AvailablePosition == null)
                    return Player.Notification(frp.Globals.messages.THERE_IS_NO_PLACE_FOR_VEHICLE, frp.Globals.Notification.Error, 5);
                Vehicle = frp.Vehicles.CreateTemporary(Configuration.Vehicle.Model, AvailablePosition.Position, AvailablePosition.Rotation, Configuration.Vehicle.Color, 'TX' + frp.Main.GenerateNumber(4));
                ...
                if (!Vehicle.numberPlate.includes('TX'))
                    return Player.Notification(frp.Globals.messages.NO_TX_IN_NUMBERPLATE, frp.Globals.Notification.Error, 5);
            }
            ...
            Shift.EnterVehicle = (Player, Vehicle, Seat) => {
                if (Shift.Vehicle == Vehicle && Shift.Driver != Player && Seat != 0) {
                    Player.SendMessage(frp.Globals.messages.YOU_ENTERED_TAXI_VEHICLE + Vehicle.numberPlate + frp.Globals.messages.PRICE_PER_MINUTE + frp.Main.Dollars(Shift.Fare) + '.', frp.Globals.Colors.white[2]);
        ...
        static async Stop(Player) {
            if (!Player.getVariable('Job_Duty'))
                return Player.Notification(frp.Globals.messages.JOB_NOT_STARTED, frp.Globals.Notification.Error, 5);
            const Character = await Player.Character();
        ...
        static Fare(Player, Price) {
            ...
            if (Shift) {
                ...
                if (Price > Max || Price < Min)
                    return Player.Notification(frp.Globals.messages.TAXIMETRE_PRICE_RANGE + Min + ' - ' + Max + '.', frp.Globals.Notification.Error, 6);
                Shift.Fare = Price;
