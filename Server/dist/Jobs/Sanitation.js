...
frp.Sanitation = class Sanitation {
    static async Stop(Player) {
        ...
        if (Player.getVariable('Job_Vehicle')) {
            ...
            if (Vehicle.dist(frp.Jobs.Job[frp.Globals.Jobs.Sanitation].position) > 25)
                return Player.Notification(frp.Globals.messages.JOB_VEHICLE_RETURN_BACK, frp.Globals.Notification.Error, 5);
    ...
    static async Start(Player) {
        if (Player.getVariable('Job_Duty'))
            return Player.Notification(frp.Globals.messages.JOB_ALREADY_STARTED, frp.Globals.Notification.Error, 5);
        ...
        const Vehicle = frp.Vehicles.CreateTemporary(Configuration.Vehicle.Model, AvailablePosition, Configuration.Vehicle.Rotation, Configuration.Vehicle.Color, 'FO0' + frp.Main.GenerateNumber(3));
