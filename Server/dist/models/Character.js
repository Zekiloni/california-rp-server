...
frp.Characters.prototype.RentVehicle = function (player, model, business, minutes = 30) {
    if (frp.Main.IsAnyVehAtPos(business.Vehicle_Point)) {
        ...
        player.notification('Mesto za isporuku vozila je trenutno zauzeto.', NOTIFY_ERROR, 4);
    }
};
...
mp.Player.prototype.Nearest = async function () {
    ...
    for (const Vehicle of Vehicles) {
        if (this.dist(Vehicle.position) < 2.25)
            return Vehicle;
    }
};
mp.Player.prototype.ProximityMessage = function (radius, message, colors) {
    mp.players.forEachInRange(this.position, radius, (target) => {
        ...
        let color = null;
        switch (true) {
            case (distance < distanceGap):
                color = colors[0];
                break;
            case (distance < distanceGap * 2):
                color = colors[1];
                break;
            case (distance < distanceGap * 3):
                color = colors[2];
                break;
            case (distance < distanceGap * 4):
                color = colors[3];
                break;
            default:
                color = colors[0];
