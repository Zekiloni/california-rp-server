...
frp.Houses.prototype.Buy = async function (Player) {
    if (this.Owner != 0)
        return Player.Notification(frp.Globals.messages.HOUSE_ALREADY_OWNER, frp.Globals.Notification.Error, 5);
...
frp.Houses.prototype.Lock = async function (Player) {
    if (this.Owner != Player.character)
        return Player.Notificationn(frp.Globals.messages.YOU_DONT_HAVE_HOUSE_KEYS, frp.Globals.Notification.Error, 5);
    if (!this.Tenants.includes(Player.character))
        return Player.Notificationn(frp.Globals.messages.YOU_DONT_HAVE_HOUSE_KEYS, frp.Globals.Notification.Error, 5);
    this.Locked = !this.Locked;
    await this.save();
};
frp.Houses.Create = async function (Player, Type, Price) {
    if (HouseTypes[Type] == undefined)
        return Player.Notification(frp.Globals.messages.TYPES_ARE_IN_RANGE + '0 - ' + HouseTypes.length + '.', frp.Globals.Notification.Error, 5);
