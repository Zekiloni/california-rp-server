...
frp.Channels.New = async function (player, frequency, password = null) {
    if (!frp.Items.HasItem(player.character, 'Handheld Radio'))
        return player.Notification(frp.Globals.messages.YOU_DONT_HAVE + ' Handheld Radio.', frp.Globals.Notification.Error, 4);
    const exist = await frp.Channels.count({ where: { Frequency: frequency } });
    if (exist)
        return player.Notification(frp.Globals.messages.CHANNEL_ALREADY_EXISTS, frp.Globals.Notification.Error, 4);
    const Character = await player.Character();
    if (Character.Frequency != 0)
        return player.Notification(frp.Globals.messages.ALREADY_IN_CHANNEL, frp.Globals.Notification.Error, 4);
...
frp.Channels.Edit = async function (player, password) {
    ...
    if (Channel.Owner != player.character)
        return player.Notification(frp.Globals.messages.NOT_ALLOWED, frp.Globals.Notification.Error, 5);
...
frp.Channels.Join = async function (player, frequency, password = null) {
    const exist = await frp.Channels.count({ where: { Frequency: frequency } });
    if (!exist)
        return player.Notification(frp.Globals.messages.CHANNEL_NOT_FOUND, frp.Globals.Notification.Error, 4);
    if (this.Password != null && this.Password != password)
        return player.Notification(frp.Globals.messages.CHANNEL_WRONG_PASSWORD, frp.Globals.Notification.Error, 4);
    const Character = await player.Character();
    Character.Frequency = frequency;
    await Character.save();
...
frp.Channels.Leave = async function (player) {
    ...
    if (Frequency.Owner == player.character)
        return player.Notification(frp.Globals.messages.NOT_ALLOWED, frp.Globals.Notification.Error, 4);
    Character.Frequency = 0;
    await Character.save();
