...
frp.Vehicles.prototype.Paint = async function (primary, secondary) {
    ...
    if (this.GameObject)
        this.GameObject.setColor(primary, secondary);
    await this.save();
};
    ...
    switch (true) {
        case this.Entity == VehicleEntities.Player: {
            if (this.Owner != Character.id)
                return Player.Notification(frp.Globals.messages.YOU_DONT_HAVE_VEHICLE_KEYS, frp.Globals.Notification.Error, 6);
        ...
        case this.Entity == VehicleEntities.Business: {
            ...
            if (Business.Workers.includes(Character.id) == false)
                return Player.Notification(frp.Globals.messages.YOU_DONT_HAVE_VEHICLE_KEYS, frp.Globals.Notification.Error, 6);
            this.GameObject.locked = !this.GameObject.locked;
            this.Locked = this.GameObject.locked;
            await this.save();
...
(async () => {
    await frp.Vehicles.sync({ force: true });
    const Vehicles = await frp.Vehicles.findAll();
