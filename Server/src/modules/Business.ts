...
//       if (Character.Money < Total) return Player.Notification(Messages.NOT_ENOUGH_MONEY, notifications.type.ERROR, 5);
...
//          if (Products[Item.name].Supplies < Item.quantity) return Player.Notification(Messages.NOT_ENOUGH_PRODUCTS, notifications.type.ERROR, 5);
...
//       if (Products[Item].Supplies < 1) return Player.Notification(Messages.NOT_ENOUGH_PRODUCTS, notifications.type.ERROR, 5);
...
//       if (Products['Clothing'].Supplies < 1) return player.Notification(Messages.NOT_ENOUGH_PRODUCTS, notifications.type.ERROR, 5);
...
//       if (Character.Money < Total) return Player.Notification(Messages.NOT_ENOUGH_MONEY, notifications.type.ERROR, 4);
//       const InBusiness = await Business.findOne({ where: { id: Biz } });
//       if (!InBusiness) return;

//       let Products = InBusiness.Products;
//       const VehiclePoint = InBusiness.Vehicle_Point;
//       if (Main.IsAnyVehAtPos(new mp.Vector3(VehiclePoint.x, VehiclePoint.y, VehiclePoint.z), 1.5).length > 0) return; Player.Notification(Messages.VEHICLE_POINT_IS_NOT_FREE, notifications.type.ERROR, 4);

      

//       console.log(Products[Model]);

//       if (Products[Model].Supplies < 1) return Player.Notification(Messages.NOT_ENOUGH_PRODUCTS, notifications.type.ERROR, 4);

//       Products[Model].Supplies --;

//       Character.GiveMoney(Player, -Total);

//       Player.Notification(Messages.SUCCESSFULLY_BUYED_VEHICLE, notifications.type.SUCCESS, 6);

//       Player.call('client:business.dealership:menu');

//       Player.Notification(Messages.NOT_ENOUGH_PRODUCTS, notifications.type.ERROR, 5);
