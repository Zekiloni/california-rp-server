...
let lpSlotMachine = null, slotMachineToJoin = null, interactingWithSlotMachine = null, canSpin = false, interactingWithSlotMachineTimeout = null, slotMachineData = [];
let SPINNING_TIME = [];
...
mp.keys.bind(0x45, true, () => // E
 {
    if (mp.gui.cursor.visible || interactingWithSlotMachine != null)
        return false;
    if (lpSlotMachine != null) {
        ...
        if (canSpin)
            canSpin = false;
        interactingWithSlotMachineTimeout = setTimeout(function () {
...
mp.events.add('playerDeath', (player) => {
    if (player == localPlayer) {
        if (lpSlotMachine != null)
            lpSlotMachine = null;
        if (interactingWithSlotMachine != null)
            interactingWithSlotMachine = null;
        if (canSpin)
            canSpin = false;
    }
