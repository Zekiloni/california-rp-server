...
let lpCasinoTable = null, casinoTableToJoin = null, casinoSeatToJoin = null, goToSeatInterval = null, interactingWithTable = null, rouletteCamera = null, canDoBets = true, betObject = null, closestChipSpot = null, interactingWithTableTimeout = null, rouletteData = [], posX = 0, posY = 0, posZ = 0;
const localPlayer = mp.players.local, tableLib = "anim_casino_b@amb@casino@games@roulette@table", dealerLib = "anim_casino_b@amb@casino@games@roulette@dealer", dealerLibF = "anim_casino_b@amb@casino@games@roulette@dealer_female";
    ...
    mp.events.add('entityStreamIn', (entity) => {
        if (entity.type == "ped" && entity.croupier != null) {
            if (entity.model == mp.game.joaat('S_M_Y_Casino_01'))
                entity.taskPlayAnim(dealerLib, "idle", 8.0, 1, -1, 1, 0.0, false, false, false);
            else
                entity.taskPlayAnim(dealerLibF, "idle", 8.0, 1, -1, 1, 0.0, false, false, false);
...
mp.events.add("rouletteAllowBets", (toggle) => {
    canDoBets = toggle;
    if (toggle)
        mp.game.graphics.notify("Place your bets.");
    else
        mp.game.graphics.notify("No more bets.");
});
    ...
    if (rouletteCamera != null && lpCasinoTable != null) {
        if (betObject != null) {
            ...
            if (mp.game.controls.isDisabledControlJustReleased(0, 24) && !mp.gui.cursor.visible) // ЛКМ
             {
                if (closestChipSpot != null)
                    mp.events.callRemote("makeRouletteBet", closestChipSpot);
            }
...
mp.events.add("spinRouletteWheel", (table, needSpins, endTable, endBall) => {
    ...
    setTimeout(function () {
        if (rouletteData[table].ped.model == mp.game.joaat('S_M_Y_Casino_01'))
            rouletteData[table].ped.taskPlayAnim(dealerLib, "idle", 8.0, 1, -1, 1, 0.0, false, false, false);
        else
            rouletteData[table].ped.taskPlayAnim(dealerLib + "_female", "idle", 8.0, 1, -1, 1, 0.0, false, false, false);
    }, 8000);
});
mp.events.add("clearRouletteTable", (table) => {
    if (rouletteData[table].ped.model == mp.game.joaat('S_M_Y_Casino_01'))
        rouletteData[table].ped.taskPlayAnim(dealerLib, "clear_chips_zone2", 8.0, 1, -1, 2, 0.0, false, false, false);
    else
        rouletteData[table].ped.taskPlayAnim(dealerLib + "_female", "clear_chips_zone2", 8.0, 1, -1, 2, 0.0, false, false, false);
    setTimeout(function () {
        if (rouletteData[table].ped.model == mp.game.joaat('S_M_Y_Casino_01'))
            rouletteData[table].ped.taskPlayAnim(dealerLib, "idle", 8.0, 1, -1, 1, 0.0, false, false, false);
        else
...
mp.keys.bind(0x45, true, () => // E
 {
    if (mp.gui.cursor.visible || interactingWithTable != null)
        return false;
    if (lpCasinoTable != null) {
        ...
        if (canDoBets)
            canDoBets = false;
        interactingWithTableTimeout = setTimeout(function () {
            interactingWithTable = null;
...
mp.keys.bind(0x46, true, () => // F
 {
    if (interactingWithTable != null || lpCasinoTable == null)
        return;
    if (rouletteCamera != null) {
        destroyRouletteCamera();
    }
...
function rouletteNumber(rouletteId) {
    ...
    if (random < 0 || random > 38) {
        rouletteNumber(rouletteId);
        return;
    }
