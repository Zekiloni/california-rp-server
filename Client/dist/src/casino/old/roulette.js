"use strict";
let canDoBets = true, betObject = null, closestChipSpot = null, rouletteTable = null, rouletteSeat = null, startRoullete = false, roulleteCam = false, blockControls = false, setBet = false;
CasinoPedsID = [],
    rouletteCamera = null,
    betCoords = null,
    RouletteTables = [];
...
mp.events.add('render', () => {
    ...
    if (setBet && rouletteCamera != null && rouletteTable != null && startRoullete == false && !mp.gui.cursor.visible) {
            ...
            if (mp.game.controls.isDisabledControlJustReleased(0, 25) && !mp.gui.cursor.visible) // ПКМ
             {
                if (closestChipSpot != null)
                    mp.events.call('bet_roulette', rouletteTable, closestChipSpot); //mp.events.callRemote("server_remove_roulette_bet", closestChipSpot);
            }
            if (mp.game.controls.isDisabledControlJustReleased(0, 24) && !mp.gui.cursor.visible) // ЛКМ
             {
                if (closestChipSpot != null)
                    mp.events.call('bet_roulette', rouletteTable, closestChipSpot); //mp.events.callRemote("server_make_roulette_bet", closestChipSpot);
            }
        ...
        if (rotx < -70)
            rotx = -70;
        if (rotz < RouletteCameraRotStop[rouletteTable][0])
            rotz = RouletteCameraRotStop[rouletteTable][0];
        if (rotz > RouletteCameraRotStop[rouletteTable][1])
            rotz = RouletteCameraRotStop[rouletteTable][1];
