...
mp.events.add({
    ...
    'playerEnterVehicle': (vehicle, seat) => {
        if (seat == -1) {
            if (browser)
                browser.execute('hud.Vehicle(true);');
        }
    },
    'playerLeaveVehicle': (vehicle, seat) => {
        if (seat == -1) {
            if (browser)
                browser.execute('hud.Vehicle(false);');
        }
    },
...
function Notify(message, type, time = 4) {
    if (browser)
        browser.execute('hud.Notification(\"' + message + '\", \"' + type + '\", ' + time + ');');
}
function Instructions(content, time = 4) {
    if (browser)
        browser.execute('hud.Instructions(\"' + content + '\", ' + time + ');');
}
...
function LocationUpdate() {
    const path = mp.game.pathfind.getStreetNameAtCoord(Player.position.x, Player.position.y, Player.position.z, 0, 0), Zone = mp.game.gxt.get(mp.game.zone.getNameOfZone(Player.position.x, Player.position.y, Player.position.z)), Street = mp.game.ui.getStreetNameFromHashKey(path.streetName), Heading = getHeading();
