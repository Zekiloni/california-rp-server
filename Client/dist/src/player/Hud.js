...
let screenshotBrowser = false, photoName = null, ScreenShotTimer = false;
mp.events.add({
...
updatePlayerHud = () => {
    let street = mp.game.pathfind.getStreetNameAtCoord(player.position.x, player.position.y, player.position.z, 0, 0), zoneName = mp.game.gxt.get(mp.game.zone.getNameOfZone(player.position.x, player.position.y, player.position.z)), streetName = mp.game.ui.getStreetNameFromHashKey(street.streetName), heading = getPlayerHeading();
    playerHUD.execute(`hud.location.street = \"${streetName}\", hud.location.zone = \"${zoneName}\", 
