    ...
    'client:player.miner:mine': () => {
        ...
        let timer = setInterval(() => {
            if (Player.getAnimCurrentTime('amb@world_human_const_drill@male@drill@base', 'base') > 0.95) {
                ...
                timer = null;
                if (!timer)
                    browser.execute('mining.clicked = false');
            }
        }, 50);
    }
});
...
function Next() {
    ...
    current >= Spots.length ? (Finish()) : (next.checkpoint = mp.checkpoints.new(47, position, 2.5, { color: [241, 224, 90, 250], visible: true, dimension: player.dimension }));
}
function Finish() {
    current = null;
    Spots = null;
