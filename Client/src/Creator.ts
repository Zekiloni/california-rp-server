...
mp.events.add(
   {
      ...
      'client:player.character.creator:gender': (x: number) => {
         Player.model = Genders[x];
      },

      'client:player.character.creator:eyes': (eyeColor: number) => {
         Player.setEyeColor(eyeColor);
      },

      'client:player.character.creator:hair': (i: number, x: number) => {
         Hair[i] = x;
         switch (i) {
            case 0: { if (x == 23 || x == 24) return; Player.setComponentVariation(2, x, 0, 0); break; }
            case 1: { Player.setHairColor(x, Hair[2]); break; }
            case 2: { Player.setHairColor(Hair[1], x); break; }
         }
      },
      ...
      'client:player.character.creator:face': (i: number, x: number) => {
