...
class House { 
   constructor (id, data) { 
      ...
      this.interior_dimension = data.intDimension || this.id;
      this.ipl = data.ipl;
   ...
   refresh () { 
      let colors = [1, 2]
      this.owner == -1 ? ( this.blip.color = colors[0] ) : ( this.blip.color = colors[1] );
...
class Houses { 
      ...
      mp.events.add({
         'playerEnterColshape': (player, shape) => { 
            ...
            if (shape.house) { 
               ...
               if (house && house.dimension == player.dimension) { 
                  if (house.owner != -1) { 
   ...
   load = () => { 
      ...
      db.query("SELECT * from `houses`", function(error, results, fields) { 
         ...
         results.forEach(result => {
            let house = new House(result.id, { entrance: JSON.parse(result.entrance), owner: result.owner, type: result.type, price: result.price, dimension: result.dimension, ipl: result.ipl, interior: result.interior, intDimension: result.intDimension })
            house.refresh();
            console.log(house)
            counter ++;
         });
   ...
   update = (house) => { 
      let values = { 
         ...
         entrance: JSON.stringify(house.entrance),
         interior: JSON.stringify(house.interior),
         ipl: house.ipl
      }

      db.query("UPDATE `houses` SET ? WHERE `id` = ?", [values, house.id], function (error, results, fields) {
